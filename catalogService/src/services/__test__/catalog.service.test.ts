    ...
    describe("update Product", ()=>{
        test("should update the product",async()=>{
            const service = new CatalogService(repository);
                ...
                const result =  await service.updateProduct(testProduct.id,testProduct);
                expect(result).toMatchObject({
                    id:expect.any(Number),
                    name:expect.any(String),
                    price:expect.any(Number),
        ...
        const service = new CatalogService(repository);
                 ...
                 await expect(service.updateProduct(testProduct.id,testProduct)).rejects.toThrow("Id does not exist hence unable to update the product");
        })
        ...
        const service = new CatalogService(repository);
                 ...
                 await expect(service.updateProduct(testProduct.id,testProduct)).rejects.toThrow("unable to create Product due to database error");
    ...
    describe("get Product", ()=>{
        test("should get the product",async()=>{
            const service = new CatalogService(repository);
                ...
                const result =  await service.getProduct(testProduct.id);
                expect(result).toMatchObject({
                    id:expect.any(Number),
                    name:expect.any(String),
                    price:expect.any(Number),
        ...
        const service = new CatalogService(repository);
                 ...
                 await expect(service.getProduct(testProduct.id)).rejects.toThrow("Id does not exist hence unable to get the product");
        })
        ...
        const service = new CatalogService(repository);
                 ...
                 await expect(service.getProduct(testProduct.id)).rejects.toThrow("Error in DB when fetching the product");
